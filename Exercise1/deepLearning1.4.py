...
class FullyConnectedLayer(Layer, Parameterized):
    ...
    def bprop(self, output_grad):
        ...
        #print "shdfgfhjfhg"
        #print self.last_input.T
        #print gradient_wrt_net
        self.dW = np.dot(self.last_input.T, gradient_wrt_net)
        
        self.db = np.dot(np.ones((1,n)), gradient_wrt_net)
        # the gradient wrt. the input should be calculated here
        
        return np.dot(gradient_wrt_net, self.W.T)
...
class NeuralNetwork:
    ...
    def sgd_epoch(self, X, Y, learning_rate, batch_size):
        ...
        numRowsX = X.shape[0]
        Xbatches = np.array_split(X, n_batches)
        Ybatches = np.array_split(Y, n_batches)
        ...
        for i in range (0,len(Xbatches)):
            ...
            for l in range (1, len(self.layers)-1):
                ...
                new_dW, newdb = self.layers[l].grad_params()
                self.layers[l].W = old_W - learning_rate * new_dW
                self.layers[l].b = old_b - learning_rate * newdb
    ...
    def train(self, X, Y, X_valid, y_valid, learning_rate=0.1, max_epochs=100, 
        ...
        for e in range(max_epochs+1):
            ...
            vError = self.classification_error(X_valid, y_valid)
            print('epoch {:.4f}, validation error {:.4f}%'.format(e, vError*100))
            ...
            if isinstance(layer, Parameterized):
                ...
                for p, param in enumerate(layer.params()):
                    ...
                    for i in range(len(param_init)):
                        epsilon_i[i] = 1
                        epsilon_incremental = epsilon_i * epsilon
                        gparam_fd[i] = (output_given_params(param_init + epsilon_incremental) - loss_base) / epsilon
                        epsilon_i[i] = 0
